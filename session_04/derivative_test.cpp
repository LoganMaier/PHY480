...
// define structure with both variables
typedef struct{
	double alpha;
	double beta;
} parameters;
...
main (void)
{
  ...
  out << "# log10(h) | log10(rel err fd) | log10(rel err cd) | log10(rel err extrap)" << endl;

  params_ptr = &params;		// double to pass to function (now with
  // both parameters 
...
// changing return to new expression
  //return (exp (-alpha * x));
