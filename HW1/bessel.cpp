...
double diff(double x, int n, int m);
  ...
  for (double x = xmin; x <= xmax; x += step)
    {
      ...
      double difference = diff(x, order, start);
      

      my_out << fixed << setprecision (10) << setw (8) << x << " "
	<< scientific << setprecision (10)
	<< setw (13) << ans_down << " "
	     << setw (13) << ans_up<< "  "  << setw(13) << difference
        << endl;
    }
...
double diff(double x, int n, int m){
  ...
  return((abs(up_recursion(x,n) - down_recursion(x,n,m)))/(0.5*(abs(up_recursion(x,n)) + abs(down_recursion(x,n,m)))));
