                  ...
                  double (*integrand) (double x) )
{
    ...
    for (int n=2; n<num_pts; n+=2)     // odd points
    {
        double x = x_min + interval * double(n-1);
        sum += (64./45.)*interval * integrand(x);
    }
    for (int n=3; n<num_pts; n+=4)    // even points
    {
        double x = x_min + interval * double(n-1);
        sum += (24./45.)*interval * integrand(x);}
    for (int n=5; n<num_pts; n+=4)          // odd points
    {
        double x = x_min + interval * double(n-1);
...
double gsl_approach(double x_min, double x_max,
                    double (*integrand) (double x, void * parameters))
{
    ...
    gsl_integration_qags (&F, x_min, x_max, 1e-7, 1e-7, 1000, w, &result, &error);
