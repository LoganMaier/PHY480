...
main (void)
{
  ...
  for (int j = 0; j <= 3; ++j){
    ...
    // construct filename
    string j_to_string = to_string(j);
    filename = "file_" + j_to_string + ".out";
    
    // open filestream
    ofstream file_j;
    file_j.open(filename.c_str());
    
    // output file number to file
    file_j << "This is file " << j_to_string << endl;
    file_j.close(); // close stream
  }

  
  ofstream file_alpha;
   
  double alpha = 5.21934;
  double round_alpha = round(alpha*pow(10,2))/pow(10,2); 
  string alpha_to_string = to_string(round_alpha);
  string final_string = string(1,alpha_to_string[0]) + string(1,alpha_to_string[1]) + string(1,alpha_to_string[2]) + string(1,alpha_to_string[3]);
  
  string file_alpha_string = "file_" + final_string + ".out";
 
  file_alpha.open(file_alpha_string.c_str());
  file_alpha << "This file includes rounded alpha value: " + final_string << endl;
  file_alpha.close();
